...
export class StorageService {
    ...
    count(tableName: string, obj?: IDBValidKey | IDBKeyRange) {
        return this.db.count(tableName, obj)
            .pipe(catchError((err, caught) => caught));
    }

    countByIndex(tableName: string, indexName: string, obj?: IDBValidKey | IDBKeyRange) {
        return this.db.countByIndex(tableName, indexName, obj)
            .pipe(catchError((err, caught) => caught));
    }
    ...
    getByIndex<T>(tableName: string, indexName: string, key: IDBValidKey) {
        return this.db.getByIndex<T>(tableName, indexName, key)
            .pipe(catchError((err, caught) => caught));
    }
